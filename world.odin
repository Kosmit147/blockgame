...
world_raycast :: proc(
  ...
  ray: Ray,
  max_distance: f32,
) -> (block: ^Block, block_position: Grid_World_Position, place_block_offset: Grid_World_Position, hit := false) {
  grid_traversal_direction := [3]i32{
    1 if ray.direction.x >= 0 else -1,
    1 if ray.direction.y >= 0 else -1,
    1 if ray.direction.z >= 0 else -1,
  }

  grid_boundary_increment := [3]i32{
    1 if ray.direction.x >= 0 else 0,
...
to_grid_world_position :: proc(
  position: Grid_Chunk_Position,
  chunk_coordinate: Chunk_Coordinate
) -> Grid_World_Position {
  return {
    chunk_coordinate.x * CHUNK_SIZE.x + position.x,
    position.y,
...
@(rodata) g_block_faces := [World_Direction][4]Chunk_Mesh_Vertex_Input_Data{
  // Front wall.
  .Plus_Z = {
...
@(rodata) g_block_indices := [6]Chunk_Mesh_Index{ 0, 1, 2, 0, 2, 3 }
